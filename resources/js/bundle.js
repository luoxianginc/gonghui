...
function createAlertObj(options) {
	var alertObj = $('<div></div>').addClass('alert alert-' + options.type + ' alert-dismissable').attr({
		role: 'alert'
	}).css({
...
function animateShow(obj, type, duration) {
	obj.addClass('animated ' + type).css('animation-duration', duration).show();
}

function animateRemove(obj, type, duration) {
	obj.addClass('animated ' + type).css('animation-duration', duration).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
		$(this).remove();
	});
}
